...
return false;},getRequestedDirectory:function(){return!_.isUndefined(window.MediabrowserUtility)&&window.MediabrowserUtility.pathId!==''?Base64.idDecode(window.MediabrowserUtility.pathId):null;},isFilterApplied:function(currentFilterPath){return!_.isUndefined(currentFilterPath)&&currentFilterPath!=='';},locateNode:function(path){if($(this.directoryTreeSelector).jstree('is_selected',path)){return;}
$(this.directoryTreeSelector).jstree('deselect_node',$(this.directoryTreeSelector).jstree('get_selected'));$(this.directoryTreeSelector).jstree('open_node',path);$(this.directoryTreeSelector).jstree('select_node',path,true);},clearFiltersHandle:function(){$(this.directoryTreeSelector).jstree('deselect_all');this.activeNode(null);this.directories().setInActive();},setActiveNodeFilter:function(nodePath){if(this.activeNode()===nodePath&&!this.jsTreeReloaded){this.selectStorageRoot();}else{this.selectFolder(nodePath);}},selectStorageRoot:function(){var filters={},applied=this.filterChips().get('applied');$(this.directoryTreeSelector).jstree('deselect_all');filters=$.extend(true,filters,applied);delete filters.path;this.filterChips().set('applied',filters);this.activeNode(null);this.waitForCondition(function(){return _.isUndefined(this.directories());}.bind(this),function(){this.directories().setInActive();}.bind(this));},selectFolder:function(path){this.activeNode(path);this.waitForCondition(function(){return _.isUndefined(this.directories());}.bind(this),function(){this.directories().setActive(path);}.bind(this));this.applyFilter(path);},removeNode:function(){$(this.directoryTreeSelector).jstree('delete_node',$(this.directoryTreeSelector).jstree('get_selected'));},applyFilter:function(path){var filters={},applied=this.filterChips().get('applied');filters=$.extend(true,filters,applied);filters.path=path;this.filterChips().set('applied',filters);},reloadJsTree:function(){var deferred=$.Deferred();this.getJsonTree().then(function(data){$(this.directoryTreeSelector).jstree(true).settings.core.data=data;$(this.directoryTreeSelector).jstree(true).refresh(false,true);this.setJsTreeReloaded(true);deferred.resolve();}.bind(this));return deferred.promise();},getJsonTree:function(){var deferred=$.Deferred();$.ajax({url:this.getDirectoryTreeUrl,type:'GET',dataType:'json',success:function(data){deferred.resolve(data);},error:function(jqXHR,textStatus){deferred.reject();throw textStatus;}});return deferred.promise();},createTree:function(data){$(this.directoryTreeSelector).jstree({plugins:[],checkbox:{three_state:false,cascade:''},core:{data:data,check_callback:true,themes:{dots:false}}});}});});